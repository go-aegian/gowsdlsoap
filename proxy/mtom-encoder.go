...
func (b *Binary) MarshalXML(enc *xml.Encoder, start xml.StartElement) error {
	...
	return enc.EncodeElement(struct {
		...
		Include: &xopPlaceholder{Href: fmt.Sprintf("cid:%s", b.packageID)},
	}, start)
...
func newMtomEncoder(w io.Writer) *mtomEncoder {
	return &mtomEncoder{writer: multipart.NewWriter(w)}
}
...
func (e *mtomEncoder) Encode(v interface{}) error {
	...
	h.Set(soap.ContentTypeHeader, `application/xop+xml`)
	h.Set(soap.ContentTransferEncodingHeader, "8bit")
	...
	for _, fld := range binaryFields {
		...
		h := make(textproto.MIMEHeader)
		h.Set(soap.ContentTypeHeader, pkg.contentType)
		h.Set(soap.ContentIdHeader, fmt.Sprintf("<%s>", pkg.packageID))
		h.Set(soap.ContentTransferEncodingHeader, "binary")
