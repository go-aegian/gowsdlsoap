...
func WithNTLM(domain, user, password string) Option {
	return func(o *options) {
		...
		o.transport = &httpntlm.NtlmTransport{
			...
			User:         user,
			Password:     password,
			RoundTripper: &http.Transport{TLSClientConfig: o.tlsCfg},
		}
	}
}
...
func (s *Client) call(ctx context.Context, soapAction string, request, response interface{}, faultDetail FaultError,
	...
	req.Header.Set("User-Agent", "gosoap/0.1")
	...
	if client == nil {
		if s.opts.transport != nil {
			s.opts.transport.RoundTripper.(*http.Transport).TLSHandshakeTimeout = s.opts.tlshshaketimeout
			if s.opts.tlsCfg != nil {
				s.opts.transport.RoundTripper.(*http.Transport).TLSClientConfig = s.opts.tlsCfg
			}
			client = &http.Client{Timeout: s.opts.contimeout, Transport: s.opts.transport}
		} else {
			tr := &http.Transport{
