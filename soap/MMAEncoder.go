...
const mmaContentType string = `multipart/related; start="<soaprequest@gosoap.lib>"; type="text/xml"; boundary="%s"`
...
func (e *mmaEncoder) Encode(v interface{}) error {
	...
	headers.Set("Content-ID", "<soaprequest@gosoap.lib>")
...
func getMmaHeader(contentType string) (string, error) {
	...
	if strings.HasPrefix(mediaType, "multipart/") {
		...
		if !ok || startInfo != "<soaprequest@gosoap.lib>" {
			return "", fmt.Errorf(`expected param start="<soaprequest@gosoap.lib>", got %s`, startInfo)
		}
		return boundary, nil
	}
